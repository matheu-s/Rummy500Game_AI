...
class Board:
    ...
    def start_board(self):
        ...
        # Dealing cards
        self.human_hand = Hand(self.deck.deal(13))
        self.engine_hand = Hand(self.deck.deal(13))
        ...
        self.button_organize = Button(image=None, pos=(WIDTH - 300, HEIGHT - 50),
                              text_input="Organize", font=get_font(20), base_color="White", hovering_color="Blue")
    ...
    def render_cards(self, player):
        ...
        if player == 'engine':
            ...
            for i in self.engine_hand.cards:
                card = pygame.transform.scale(hidden_img, (int(card_width), int(card_height)))
                self.screen.blit(card, (600 + self.engine_hand.pos, 20))
                self.engine_hand.pos += 25
        elif player == 'human':
            for i in self.human_hand.cards:
                card = pygame.transform.scale(i.image, (int(card_width), int(card_height)))
                self.screen.blit(card, (600 + self.human_hand.pos, (HEIGHT) - (card_height + 20)))
                self.human_hand.pos += 25
        else:
            # Render to the pile
            for i in self.pile:
                card = pygame.transform.scale(i.image, (int(card_width), int(card_height)))
